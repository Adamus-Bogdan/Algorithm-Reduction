...
- File [`main-QQ.py`](https://github.com/Adamus-Bogdan/Algorithm-Reduction/blob/master/main-QQ.py) contains full program which inverts the mapping _F_ (defined in example 17 in our paper) using our base approach. To run this program use the following command:
```bash
$ sage main-QQ.py F
```
...
# Comparison to Groebner-basis algorithm

We compare our improved algorithm (eg. containing reductions modulo prime numbers) to algorithm based on Groebner basis described in [[4]]().
...
We run program using our approach for the mapping _W_ defined in example 18 in our article:
...
For mapping _W_ our approach appeared to be more effective.
